...
class PostgresRepository {
  ...
  async replaceResetToken(token) {
    ...
    await this.pool.query(
      `insert into reset_tokens (id, user_id, email, code, expires_at, created_at)
       values ($1,$2,$3,$4,$5,$6)`,
      [token.id, token.userId, token.email, token.code, token.expiresAt, token.createdAt],
    );
  }

  async getValidResetToken(email, code, nowIso) {
    await this.init();
    const result = await this.pool.query(
...
class FileRepository {
  ...
  async getValidResetToken(email,code,nowIso){return this.read().resetTokens.find((t)=>t.email===email&&t.code===code&&new Date(t.expiresAt)>new Date(nowIso))||null;}
  ...
  async upsertEmergencyContact(contact){const s=this.read(); s.emergencyContacts=s.emergencyContacts.filter((c)=>c.userId!==contact.userId); s.emergencyContacts.push(contact); this.write(s); return contact;}
